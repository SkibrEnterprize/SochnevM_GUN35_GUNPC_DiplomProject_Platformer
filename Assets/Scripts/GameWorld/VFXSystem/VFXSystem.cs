...
namespace VFX
{
    public class VFXSystem : MonoBehaviour, IVFXSystem
    {
            ...
            Transform parent)
        {
            ...
            if (vfxEvent == null || vfxEvent.effectPrefab == null)
            {
                Debug.Log($"In VFX System not found vfxEvent or vfxEffectPrefab for {type.ToString()}");
                 return;
            }
            ...
            if (instance == null)
            {
                Debug.Log($"In VFX System not found free instance for {vfxEvent.name}." +
                    $"\nPlease add value tmpty instanses");
                return;
            }
            instance.SetActive(false);
